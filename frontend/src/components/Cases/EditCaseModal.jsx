...
export default function EditCaseModal({ open, caseData, onClose, onSaved }) {
  ...
  return (
    <div style={{ display:'flex', position:'fixed', inset:0, background:'rgba(28,26,24,0.45)', zIndex:1000, alignItems:'center', justifyContent:'center', backdropFilter:'blur(3px)', padding:16 }}
      ...
      <div style={{ background:'#fff', borderRadius:18, border:'1.5px solid rgba(28,26,24,0.08)', boxShadow:'0 8px 32px rgba(28,26,24,0.12)', width:'100%', maxWidth:600, maxHeight:'92vh', overflowY:'auto', animation:'ecIn 0.2s ease' }}>

        {/* Header */}
        <div style={{ padding:'22px 26px 14px', display:'flex', alignItems:'flex-start', justifyContent:'space-between', position:'sticky', top:0, background:'#fff', zIndex:2, borderBottom:'1px solid rgba(28,26,24,0.06)' }}>
          <div>
            <h3 style={{ fontFamily:"'DM Serif Display',serif", fontSize:22, fontWeight:400 }}>Edit Case</h3>
            <div style={{ fontSize:11, fontFamily:'monospace', color:'#9B9590', marginTop:2 }}>{caseData?.caseCode}</div>
          </div>
        ...
        <div style={{ padding:'16px 26px 8px' }}>
          ...
          <div style={{ marginBottom:12 }}><label style={LBL}>Case title *</label>{fInp('title','Case title')}</div>
          <div style={{ marginBottom:12 }}><label style={LBL}>Subtitle</label>{fInp('subtitle','Brief description')}</div>

          {/* Petitioner & Respondent */}
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10 }}>
            <div style={{ marginBottom:12 }}>
              <label style={LBL}>Petitioner Name</label>
              {fInp('petitionerName','e.g. Rahul Gupta / Company')}
            </div>
            <div style={{ marginBottom:12 }}>
              <label style={LBL}>Respondent Name</label>
              {fInp('respondentName','e.g. Opposing Party')}
            </div>
          </div>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10 }}>
            <div style={{ marginBottom:12 }}>
              ...
              <SelectOrOther
                ...
                value={lawyers.find(l=>l._id===form.lawyer)?.name || (typeof form.lawyer === 'string' && !form.lawyer.match(/^[0-9a-f]{24}$/) ? form.lawyer : '')}
                onChange={(v) => {
            ...
            <div style={{ marginBottom:12 }}><label style={LBL}>Opposing counsel</label>{fInp('opposingCounsel','Adv. Name')}</div>
          </div>
          ...
          <div style={{ marginBottom:12 }}><label style={LBL}>Relief sought</label>{fInp('reliefByPlaintiff','e.g. Recovery of ₹XX')}</div>
          <div style={{ marginBottom:12 }}><label style={LBL}>Our position</label>{fInp('ourPosition','e.g. Dismiss the suit')}</div>
          <div style={{ marginBottom:12 }}>
            <label style={LBL}>Strategy & remarks</label>
            <textarea rows={3} style={{ ...INP, resize:'vertical' }} value={form.strategyRemarks||''} onChange={set('strategyRemarks')} placeholder="Key facts, risks, action items…"
      ...
      <style>{`@keyframes ecIn{from{opacity:0;transform:translateY(14px) scale(0.98);}to{opacity:1;transform:none;}}@keyframes ecSpin{to{transform:rotate(360deg);}}`}</style>
    </div>
  );
}
